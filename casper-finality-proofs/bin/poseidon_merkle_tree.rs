use casper_finality_proofs::utils::{
    poseidon_helpers::{binary_to_hex, ValidatorPoseidonDataOutput},
    poseidon_merkle_tree::{
        compute_merkle_hash_tree, get_validator_proof, gindex_from_validator_index,
        parse_validator_data, prove_validator_membership, zero_hashes, MAX_DEPTH,
    },
};
use num_bigint::BigUint;
use plonky2::{
    field::goldilocks_field::GoldilocksField,
    hash::{hash_types::HashOut, poseidon::PoseidonPermutation},
};
use serde::Serialize;
...
pub fn main() {
    let file_path_toy_data = "all_validators_234400.json";
    let file_path_toy_data_out = "all_validators_234400_poseidon.json";
...
pub fn compute_all_validators_tree(file_path: &str) -> PoseidonData {
    ...
    for i in 0..validators_hashed.len() {
        ...
        poseidon_validator_obj_vec.push(ValidatorPoseidonDataOutput {
            trusted: validators_raw[i].trusted,
            // validator_index: validators_raw[i].validator_index,
            // validator_index: validator_gindex,
            validator_index: i as u64,

            activation_eligibility_epoch: biguint_to_u64_unsafe(
                validators_raw[i].activation_eligibility_epoch.clone(),
            ),
            ...
            withdrawal_credentials: binary_to_hex(
                validators_raw[i].withdrawal_credentials.as_slice(),
            ),
