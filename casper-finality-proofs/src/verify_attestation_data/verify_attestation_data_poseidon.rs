use plonky2x::{
    ...
    frontend::{
        eth::vars::BLSPubkeyVariable, hash::poseidon::poseidon256::PoseidonHashOutVariable, uint::uint64::U64Variable, vars::{BoolVariable, ByteVariable, Bytes32Variable, BytesVariable}
    },
    prelude::{CircuitBuilder, Field, PlonkParameters},
};

use crate::{constants::VALIDATORS_HASH_TREE_DEPTH, utils::{eth_objects::{AttestationPoseidon, ValidatorDataPoseidon}, poseidon::ssz_verify_proof_poseidon}};
use crate::{
    constants::{STATE_ROOT_GINDEX, TEST_VALIDATORS_IN_COMMITMENT_SIZE, VALIDATOR_ROOT_GINDEX},
    utils::eth_objects::{Attestation, BeaconValidatorVariable, ValidatorData},
};
#[derive(Debug, Clone)]
pub struct VerifyAttestationDataPoseidon;

impl VerifyAttestationDataPoseidon {
            ...
            plonky2::plonk::config::AlgebraicHasher<<L as PlonkParameters<D>>::Field>,
    {
        ...
        bls_accumulator = accumulate_bls(
            builder,
            bls_accumulator,
            first_validator.beacon_validator_variable.pubkey,
        );
        ...
        builder.write(attestation.data.slot);
...
fn verify_validator<L: PlonkParameters<D>, const D: usize>(
    // TODO: Should pass only trusted_validators
    builder: &mut CircuitBuilder<L, D>,
    validator: ValidatorDataPoseidon,
        ...
        plonky2::plonk::config::AlgebraicHasher<<L as PlonkParameters<D>>::Field>,
{
    ...
    let valid_epochs = vec![
        align_epoch1.variable,
        align_epoch2.variable,
...
*/
pub fn hash_validator<L: PlonkParameters<D>, const D: usize>(
    builder: &mut CircuitBuilder<L, D>,
    ...
    let mut slashed_bytes32 = Bytes32Variable(BytesVariable::<32>(
        vec![builder.zero::<ByteVariable>(); 32].try_into().unwrap(),
    ));
    let slashed_byte_variable =
        ByteVariable([validator.slashed, zero, zero, zero, zero, zero, zero, zero]);
    slashed_bytes32.0 .0[0] = slashed_byte_variable;

    let activation_eligibility_epoch_bytes32 =
        builder.ssz_hash_tree_root(validator.activation_eligibility_epoch);
    ...
    let digest_aee_ae: Bytes32Variable = builder.curta_sha256_pair(
        activation_eligibility_epoch_bytes32,
        activation_epoch_bytes32,
    );
...
fn accumulate_bls<L: PlonkParameters<D>, const D: usize>(
    //TODO:
    builder: &mut CircuitBuilder<L, D>,
...
fn init_bls<L: PlonkParameters<D>, const D: usize>(
    // Definition may change
    builder: &mut CircuitBuilder<L, D>,
    message: Bytes32Variable, // source + target
    private_key: BLSPubkeyVariable,
) -> BLSPubkeyVariable {
    // private_key.hash_to_signature()
