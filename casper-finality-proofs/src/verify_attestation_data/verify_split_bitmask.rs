...
mod utils;
use utils::variable::{variable_int_div_rem, variable_int_rem};

const TOTAL_BITMASK_SIZE: usize = 13_600_000; // MUST BE DIVISIBLE BY 63 
const BITMASK_PARTITIONS: usize = 400;
const RANGE_SIZE: usize = TOTAL_BITMASK_SIZE / BITMASK_PARTITIONS; //This must be divisible by 63

const VALIDATORS_PER_COMMITTEE: usize = 128; // 2048 MAX
const BITS_IN_FIELD: usize = 64; 
const PACKED_VALIDATOR_COUNT: usize = RANGE_SIZE.div_ceil(BITS_IN_FIELD); 
...
impl Circuit for ValidatorBitmask {
    fn define<L: PlonkParameters<D>, const D: usize>(builder: &mut CircuitBuilder<L, D>) {
        ...
        let zero_field = 
            <L as PlonkParameters<D>>::Field::from_canonical_u64(0);
        ...
        let range_size: Variable = builder.constant(<L as PlonkParameters<D>>::Field::from_canonical_usize(RANGE_SIZE));
        ...
        packed_bitmask_vec.resize(PACKED_VALIDATOR_COUNT, builder.constant(zero_field));
        ...
        let pow2_consts = get_pow2_arr(builder, zero_field);

        // Instantiate packed bitmask
        for cur_validator in validators_vec {
            for packed_idx in 0..PACKED_VALIDATOR_COUNT {

                let is_validator_in_partition = is_valid_validator_range(
                    builder,
                    range_begin,
                    range_end,
                    cur_validator
                ...
                let partition_idx = builder.sub(
                    cur_validator,
                    range_begin
                ); // Index in Partition (if valid)

                let (quotient,cur_bit) = variable_int_div_rem(
                    builder,
                    partition_idx,
                    bits_in_field_const
                ); // quotient := Packed Partition Index; cur_bit := Bit to set in Field element
                ...
                let correct_idx_and_valid_partition =  builder.and(are_at_idx,is_validator_in_partition);
                let value_to_add = builder.select(correct_idx_and_valid_partition ,cur_pow2, zero_u64);
                packed_bitmask_vec[packed_idx] = builder.add(packed_bitmask_vec[packed_idx], value_to_add);

                // Carry-over vulnerability check
                let is_counted = variable_int_rem(builder, packed_bitmask_vec[packed_idx],cur_pow2);
        ...
        let packed_bitmask: ArrayVariable<Variable, PACKED_VALIDATOR_COUNT> = ArrayVariable::new(packed_bitmask_vec);
...
fn get_pow2_arr<L: PlonkParameters<D>, const D: usize> (
    builder: &mut CircuitBuilder<L, D>,
    zero_field:<L as PlonkParameters<D>>::Field
) -> Vec<Variable> {
    let mut pow2_consts: Vec<Variable> = Vec::with_capacity(BITS_IN_FIELD);
    pow2_consts.resize(BITS_IN_FIELD, builder.constant(zero_field));
    for i in 0..BITS_IN_FIELD {
        let cur_pow = (2 as u64).pow(i as u32);
        let cur_pow_field = 
            <L as PlonkParameters<D>>::Field::from_canonical_u64(cur_pow);
