...
pub fn parse_validator_data(file_path: &str) -> (Vec<Validator>, Vec<HashOut<GoldilocksField>>) {
    let validators_raw: Vec<Validator> = read_validator_data(file_path).to_vec();

    println!("Total number of validators - {}", validators_raw.len());

    let mut validators_hashed: Vec<HashOut<GoldilocksField>> =
        Vec::with_capacity(validators_raw.len());

    for i in 0..validators_raw.len() {
        let cur_validator_hash = compute_validator_poseidon_hash(validators_raw[i].clone());
...
pub fn zero_hashes() -> Vec<HashOut<GoldilocksField>> {
    ...
    // Yes this is wrong to be adjusted to the real commitment mapper (although not the cause of the current bug)
    let mut hashes = vec![
    ...
    for i in 0..ZERO_HASHES_MAX_INDEX {
        hashes[i + 1] = combine_two_hash_n_to_hash_no_pad(hashes[i], hashes[i]);
    }

    hashes
}

fn hash_bits_arr_in_goldilocks_to_hash_no_pad(validator_data: &[bool]) -> HashOut<GoldilocksField> {
...
fn hash_biguint_in_goldilocks_to_hash_no_pad(validator_data: BigUint) -> HashOut<GoldilocksField> {
...
pub fn combine_two_hash_n_to_hash_no_pad(
...
pub fn compute_validator_poseidon_hash(validator: Validator) -> HashOut<GoldilocksField> {
    let leaves = vec![
        ...
        hash_biguint_in_goldilocks_to_hash_no_pad(validator.activation_eligibility_epoch.clone()),
    ...
    let poseidon_hash_tree_root = compute_poseidon_hash_tree_root(leaves.len(), leaves.clone());
...
pub fn compute_merkle_hash_tree(
    ...
    for i in 0..leaves.len() {
        validator_map.insert(
            gindex_from_validator_index(i as u64, MAX_DEPTH as u32),
            leaves[i],
        );
    }

    let mut leaves_len = (leaves.len() + 1) / 2;

    for cur_depth in 1..MAX_DEPTH + 1 {
        println!("cur_depth {}", cur_depth);
        println!("leaves_len {}", leaves_len);

        for i in 0..leaves_len {
            let gindex = gindex_from_validator_index(i as u64, (MAX_DEPTH - cur_depth) as u32);
            let left_child_gindex = gindex * 2;
            let right_child_gindex = gindex * 2 + 1;
            let left_hash = validator_map[&left_child_gindex];
            let right_hash = if validator_map.contains_key(&right_child_gindex) {
                validator_map[&right_child_gindex]
            } else {
                zero_hashes[cur_depth - 1]
            };

            let hash = combine_two_hash_n_to_hash_no_pad(left_hash, right_hash);
            validator_map.insert(gindex, hash);
        }

        leaves_len = (leaves_len + 1) / 2;
    }
...
fn left_right_child_from_hashmap(
    ...
    let left_elem = if validator_map.contains_key(&left_key) {
        validator_map[&left_key]
    } else {
        zero_hashes[MAX_DEPTH - (depth + 1) as usize]
    };
    let right_elem = if validator_map.contains_key(&right_key) {
        validator_map[&right_key]
    } else {
        zero_hashes[MAX_DEPTH - (depth + 1) as usize]
    };
...
pub fn compute_hashes_at_depth(
    ...
    for cur_index in 0..depth {
        let (left, right) =
            left_right_child_from_hashmap(validator_map, zero_hashes, cur_index, depth);

        let cur_hash = combine_two_hash_n_to_hash_no_pad(left, right);
...
pub fn get_validator_proof(
    ...
    while gindex != 1 {
        let sibling_gindex = if gindex % 2 == 0 {
            gindex + 1
        } else {
            gindex - 1
        };
    ...
    assert!(proof.len() == MAX_DEPTH as usize);
...
pub fn prove_validator_membership(
    ...
    for idx in 0..MAX_DEPTH {
        if path[idx] {
            hash = combine_two_hash_n_to_hash_no_pad(hash, validator_proof[idx]);
        } else {
            hash = combine_two_hash_n_to_hash_no_pad(validator_proof[idx], hash);
        }
    }
...
mod tests {
    ...
    use crate::utils::poseidon_merkle_tree::{
        combine_two_hash_n_to_hash_no_pad, compute_merkle_hash_tree, get_validator_proof,
        gindex_from_validator_index, parse_validator_data, prove_validator_membership, MAX_DEPTH,
    };
    ...
    fn test_leftmost() {
        ...
        let mut validator_map = compute_merkle_hash_tree(&validators_hashed);
        ...
        let hash = combine_two_hash_n_to_hash_no_pad(validators_hashed[0], proof[0]);

        let hash2 = combine_two_hash_n_to_hash_no_pad(hash, proof[1]);
        println!(
            "Manualy Computed Root: {:?}\nGiven Root: {:?}\n",
            hash2, validator_map[&0]
        );
        assert!(hash2 == validator_map[&0]);

        prove_validator_membership(validators_hashed[0], proof.clone(), validator_map[&0], 0);
    }
    ...
    fn test_with_missing_hash() {
        ...
        let mut validators_with_missing: Vec<HashOut<GoldilocksField>> = validators_hashed;

        validators_with_missing[3] = zeroes[0];

        let validator_map = compute_merkle_hash_tree(&validators_with_missing);
    ...
    fn test_with_depth_40() {
        ...
        let validator_map = compute_merkle_hash_tree(&validators_hashed);
        ...
        for i in 0..validators_hashed.len() {
            ...
            prove_validator_membership(validators_hashed[i], proof.clone(), validator_map[&0], i);
            println!("Proof for validator-{} passed!", i);
        }
    }
}
