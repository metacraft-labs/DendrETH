{ inputs, ... }:
{
  perSystem =
    ...
    let
      ...
      packageCircuitExecuable =
        level: executable: circuit-data:
        let
          ...
          pkg = runCommand name { inherit (executable) meta; } ''
            ...
            # there.
            CIRCUIT_DIR=${
              if circuit-data == [ ] then "./serialized_circuits" else "$out/data/serialized_circuits"
            }
      ...
      mapping = {
        ...
        pubkey-commitment-mapper = {
          ...
          builderName = "pubkey_commitment_mapper_circuit_data_generation";
          range = range 0 32;
        };
        final-layer = {
          ...
          linkCircuitData =
            circuits:
            circuits.balance-verifier.circuit-data."37" ++ circuits.commitment-mapper.circuit-data."40";
        };
      };

      packageAllCircuitExecutables =
        mapping:
        let
          buildCircuitData =
            mapping:
              ...
              let
                ...
                circuit-data =
                  if builderName == null then
                    ...
                    { "0" = [ (buildCircuit circuit-builder [ 0 ]) ]; }
                  else if argsBuilder == null then
                    let
          ...
          linkCircuitData =
            ...
            mapAttrs (
              name:
              {
                linkCircuitData ? null,
                ...
              }:
              let
                circuit-info = all-deps-free-circuit-data.${name};
                linked-circuit-data =
                  if linkCircuitData == null then
                    circuit-info.circuit-data
                  else
                    linkCircuitData all-deps-free-circuit-data;
              in
              if linkCircuitData == null then
                circuit-info
              else
                circuit-info // { circuit-data."0" = linked-circuit-data; }
            ) mapping;

          linked-circuits = linkCircuitData mapping;

          circuits-with-levels = mapAttrs (
            name:
            value@{
              binary,
              circuit-data ? { },
            ...
            // {
              levels = {
                all = packageCircuitExecuable "all" binary (lib.flatten (builtins.attrValues circuit-data));
