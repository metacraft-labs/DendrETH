...
contract BeaconLightClient is BeaconChain, Bitfield, BLSVerify {
  ...
  function state_root() public view returns (bytes32) {
    return finalized_header.state_root; //gas-reporter #132
  }
    ...
    payable
  {
    ...
    require(update.finalized_header.slot > finalized_header.slot, '!new'); //gas-reporter #191

    finalized_header = update.finalized_header; //gas-reporter #193
    prev_sync_committee_root = update.next_sync_committee_root; //gas-reporter #194

    emit FinalizedHeaderImported(update.finalized_header); //gas-reporter #196
  }
  ...
  function verify_signed_header(
    ...
    bytes32 signing_root = compute_signing_root(header, domain); //gas-reporter #215
    ...
    returns (uint256)
  {
    return
      ...
      countSetBits(uint256(sync_committee_bits[2])); //gas-reporter #264
  }
}