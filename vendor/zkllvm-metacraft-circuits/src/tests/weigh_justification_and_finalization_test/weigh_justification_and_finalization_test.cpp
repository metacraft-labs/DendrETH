...
void assert_slot_is_not_first_in_epoch(
    Slot slot
)
{
...
bool is_supermajority_link(
    Gwei target_balance,
    Gwei total_active_balance
)
{
...
)
{
    ...
    auto new_current_justified_checkpoint = previous_epoch_supermajority_link_pred ? 
...
CheckpointVariable process_finalizations(
    const JustificationBitsVariable& justification_bits,
...
)
{
    ...
    const auto previous_justified_checkpoint_epoch_plus_three =
        previous_justified_checkpoint.epoch + three;
    const auto previous_justified_checkpoint_epoch_plus_two =
        previous_justified_checkpoint.epoch + two;
    const auto current_justified_checkpoint_epoch_plus_two =
        current_justified_checkpoint.epoch + two;
    const auto current_justified_checkpoint_epoch_plus_one =
        current_justified_checkpoint.epoch + one;

    const auto second_using_fourth_as_source_pred =
        previous_justified_checkpoint_epoch_plus_three ==
        current_epoch;

    const auto second_using_third_as_source_pred =
        previous_justified_checkpoint_epoch_plus_two == current_epoch;

    const auto first_using_third_as_source_pred =
        current_justified_checkpoint_epoch_plus_two == current_epoch;

    const auto first_using_second_as_source_pred =
        current_justified_checkpoint_epoch_plus_one == current_epoch;
...
)
{
    ...
    auto new_finalized_checkpoint = process_finalizations(
        new_justification_bits,
        previous_justified_checkpoint,
        current_justified_checkpoint,
        current_epoch,
...
void test_circuit_sample_data() {
    ...
    std::cout 
        << "new_previous_justified_checkpoint: "
        << expected_new_previous_justified_checkpoint
        << "\n";
    std::cout
        << "new_current_justified_checkpoint: "
        << expected_new_current_justified_checkpoint
        << "\n";
