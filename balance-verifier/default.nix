{ inputs, ... }:
{
  perSystem =
    ...
    let
      inherit (inputs.mcl-nixos-modules.inputs) crane;
      inherit (inputs'.mcl-nixos-modules.legacyPackages) nix2container pkgs-with-rust-overlay;
      ...
      balance-verifier-circuit-per-level = lib.genAttrs (
        allLevels ++ [ "all" ]
      ) balance-verification-circuit;
      ...
      buildImage =
        ...
        nix2container.buildImage {
          ...
          copyToRoot = pkgs.buildEnv {
            ...
            paths = [
              balance-verifier
              balance-verifier-circuit-per-level."${level}"
              pkgs.bash
              pkgs.coreutils
              (lib.optionalString (level != "0") balance-verifier-circuit-per-level."${levelBefore}")
            ];
            pathsToLink = [ "/bin" ];
          };
          config = {
      ...
      balance-verifier-all-images = writeScriptBin "balance-verifier-all-images" (
        lib.concatMapStringsSep "\n" (level: getExe (buildImage level).copyToDockerDaemon) allLevels
      );
    ...
    in
    {
      legacyPackages = {
        inherit
          balance-verifier-circuit-per-level
          balance-verifier-circuit-per-level-docker
          commitment-mapper-data
          ;
        inherit
          balance-verifier
