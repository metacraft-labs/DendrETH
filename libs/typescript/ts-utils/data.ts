...
export async function getInputSignature(
  pubkey: string,
  signature: string,
  blockRoot: string,
) {
  ...
  let pubkeyArray: string[][] = [
    bigint_to_array(
      55,
      7,
      BigInt('0x' + pubkeyPoint.toAffine()[0].value.toString(16)),
    ),
    bigint_to_array(
      55,
      7,
      BigInt('0x' + pubkeyPoint.toAffine()[1].value.toString(16)),
    ),
  ];
  ...
  let signatureArray: string[][][] = [
    [
      bigint_to_array(
        55,
        7,
        BigInt('0x' + signaturePoint.toAffine()[0].c0.value.toString(16)),
      ),
      bigint_to_array(
        55,
        7,
        BigInt('0x' + signaturePoint.toAffine()[0].c1.value.toString(16)),
      ),
    ],
    [
      bigint_to_array(
        55,
        7,
        BigInt('0x' + signaturePoint.toAffine()[1].c0.value.toString(16)),
      ),
      bigint_to_array(
        55,
        7,
        BigInt('0x' + signaturePoint.toAffine()[1].c1.value.toString(16)),
      ),
    ],
  ];
