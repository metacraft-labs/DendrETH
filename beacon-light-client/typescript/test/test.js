...
function hexStringToUint8Array(_hex) {
    ...
    let array = [];
    for (let i = 0; i < _hex.length; i += 2) array.push(parseInt(_hex.slice(i, i + 2), 16));
...
function formatUpdate(update) {
    ...
    update.sync_aggregate.sync_committee_bits = [...hexStringToUint8Array(update.sync_aggregate.sync_committee_bits)]
        .map((x) => x.toString(2).padStart(8, '0'))
        .join('')
        .split('')
        .map(Number);
    update.sync_aggregate.sync_committee_signature = hexStringToUint8Array(
...
function formatSnapshot(snapshot) {
    ...
    snapshot.current_sync_committee.aggregate_pubkey = hexStringToUint8Array(
...
describe('Tests', function () {
    ...
    const RANDOM_BYTES = [
        [
            0xb1, 0x0e, 0x5f, 0x99, 0xe8, 0xf9, 0xa0, 0xff, 0x42, 0x63, 0x06, 0x54, 0x1b, 0x60, 0x6e, 0x55, 0xc5, 0xcb,
            0x55, 0x51, 0xe6, 0xe5, 0x55, 0x28, 0x00, 0x60, 0xad, 0x1a, 0x36, 0x61, 0x96, 0x74,
        ],
        [
            0x41, 0xea, 0xb3, 0xe7, 0xa7, 0x06, 0x98, 0x9d, 0x60, 0xbd, 0x4a, 0x07, 0x9f, 0x42, 0xc6, 0x62, 0xb2, 0xee,
            0xae, 0x2e, 0xc4, 0xdd, 0x11, 0xeb, 0x69, 0x13, 0x9a, 0x90, 0x7f, 0x7f, 0xbb, 0x53,
        ],
        [
            0x94, 0x25, 0xfe, 0x42, 0xbf, 0xc1, 0xf4, 0x18, 0xce, 0xff, 0x56, 0x07, 0x8a, 0x25, 0x84, 0x90, 0x43, 0x68,
            0xb5, 0x49, 0x25, 0xf4, 0xbf, 0x00, 0x8a, 0x3d, 0x76, 0x9e, 0x5b, 0xb9, 0xc5, 0x91,
        ],
        [
            0x3e, 0xcb, 0xf8, 0x36, 0xb6, 0xf2, 0xdc, 0xa2, 0x49, 0xf9, 0x60, 0x41, 0xaa, 0x1d, 0x6d, 0xc3, 0xd2, 0x6b,
            0x34, 0x9c, 0x35, 0x41, 0xff, 0x85, 0x85, 0x02, 0x5b, 0x3e, 0x5f, 0x81, 0x97, 0x30,
        ],
    ];
    const RANDOM_BYTES_ROOT = [
        52, 102, 42, 239, 137, 145, 193, 146, 121, 211, 207, 70, 228, 246, 164, 172, 174, 157, 201, 52, 174, 119, 148,
        78, 75, 151, 31, 52, 86, 227, 4, 92,
    ];
    ...
    before(async function () {
        ...
        BLC_SNAPSHOT = formatSnapshot(
            (
                await axios.get(
                    'https://raw.githubusercontent.com/metacraft-labs/eth2-light-client-updates/main/mainnet/snapshot.json',
                )
            ).data.data.v,
        );
        const promises = [];
        for (let i = 291; i <= 533; i++)
            promises.push(
                axios.get(
                    `https://raw.githubusercontent.com/metacraft-labs/eth2-light-client-updates/main/mainnet/updates/${i
                        .toString()
                        .padStart(5, '0')}.json`,
                ),
            );
    ...
    describe('SP', function () {
        ...
        describe('Sp.pack', function () {
            it('should parse number into uint8 array buffer', function () {
                expect(Uint8ArrayToHexString(Sp.pack(0))).to.eq(Uint8ArrayToHexString([0]));

                expect(Uint8ArrayToHexString(Sp.pack(parseInt(72057594037927936n)))).to.eq('0x0100000000000000');
            });
        });

        describe('Sp.sha256', function () {
            it('should hash the passed value using sha256 algorythm', function () {
                expect(Uint8ArrayToHexString(Sp.sha256(EMPTY_BYTES32))).to.equal(
    ...
    describe('CONSTANTS', function () {
        ...
        describe('PHASE 0 constants', function () {
            ...
            it('GENESIS_FORK_VERSION should be 4 empty bytes long', function () {
                ...
                expect(Uint8ArrayToHexString(contracts.Constants.GENESIS_FORK_VERSION)).to.equal(
                    Uint8ArrayToHexString(EMPTY_BYTES4),
                );
            });
        ...
        describe('ALTAIR upgrade constants', function () {
            ...
            it('EMPTY_BEACON_HEADER should be empty and should have correctly sized properties', function () {
                expect(JSON.stringify(contracts.Constants.EMPTY_BEACON_HEADER)).to.equal(
                    JSON.stringify(EMPTY_BEACON_HEADER),
                );
            });

            it('EMPTY_SYNC_COMMITTEE should be empty and should have correctly sized properties', function () {
                expect(JSON.stringify(contracts.Constants.EMPTY_SYNC_COMMITTEE)).to.equal(
                    JSON.stringify(EMPTY_SYNC_COMMITTEE),
                );
            });

            it('EMPTY_LIGHT_CLIENT_UPDATE should be empty and should have correctly sized properties', function () {
                expect(JSON.stringify(contracts.Constants.EMPTY_LIGHT_CLIENT_UPDATE)).to.equal(
                    JSON.stringify(EMPTY_LIGHT_CLIENT_UPDATE),
                );
            });
        });
    });
    ...
    describe('HELPERS', function () {
        ...
        describe('getElementInBytesArrayAt', function () {
            it('should NOT throw if index is greater or equal than array size', function () {
                ...
                expect(Uint8ArrayToHexString(contracts.Helpers.getElementInBytesArrayAt(1, []))).to.equal(
                    Uint8ArrayToHexString(EMPTY_BYTES32),
                );
            });
            ...
            it('should return the right element by index', function () {
                ...
                expect(
                    contracts.Helpers.getElementInUintArrayAt(4, [
                        '0x01',
                        '0x01',
                        '0x01',
                        '0x01',
        ...
        describe('setElementInBytesArrayAt', function () {
            ...
            it('should return the right element by index', function () {
                expect(
                    JSON.stringify(
                        contracts.Helpers.setElementInBytesArrayAt(3, ['0x01', '0x02', '0x03', '0x04'], '0x00'),
                    ),
                ).to.equal(JSON.stringify(['0x01', '0x02', '0x03', '0x00']));
                expect(
                    JSON.stringify(
                        contracts.Helpers.setElementInBytesArrayAt(2, ['0x01', '0x02', '0x00', '0x04'], '0x00'),
                    ),
                ).to.equal(JSON.stringify(['0x01', '0x02', '0x00', '0x04']));
                expect(
                    JSON.stringify(
                        contracts.Helpers.setElementInBytesArrayAt(1, ['0x01', '0x00', '0x03', '0x04'], '0x00'),
                    ),
                ).to.equal(JSON.stringify(['0x01', '0x00', '0x03', '0x04']));
                expect(
                    JSON.stringify(
                        contracts.Helpers.setElementInBytesArrayAt(0, ['0x00', '0x02', '0x03', '0x04'], '0x00'),
                    ),
                ).to.equal(JSON.stringify(['0x00', '0x02', '0x03', '0x04']));

                expect(
                    JSON.stringify(
                        contracts.Helpers.setElementInBytesArrayAt(
                            4,
                            ['0x01', '0x01', '0x01', '0x01', '0x07', '0x01', '0x01', '0x01', '0x01'],
                            '0x01',
                        ),
                    ),
    ...
    describe('UTILS', function () {
        ...
        describe('to_little_endian_64', function () {
            it('should return the reverse64 value of input as bytes', function () {
                expect(Uint8ArrayToHexString(contracts.Utils.to_little_endian_64(0))).to.equal(
                    Uint8ArrayToHexString([...Array(7).fill(0), ...[0]]),
                );
                expect(
                    Uint8ArrayToHexString(contracts.Utils.to_little_endian_64(parseInt(72057594037927936n))),
                ).to.equal(Uint8ArrayToHexString([...Array(7).fill(0), ...[1]]));
                expect(
                    Uint8ArrayToHexString(contracts.Utils.to_little_endian_64(parseInt(144115188075855872n))),
        ...
        describe('compute_epoch_at_slot', function () {
            it('should return floored integer dividing the input by the SLOTS_PER_EPOCH value (32 slots/epoch)', function () {
                expect(contracts.Utils.compute_epoch_at_slot(0)).to.equal(
                    Math.floor(0 / contracts.Utils.SLOTS_PER_EPOCH),
                );
                expect(contracts.Utils.compute_epoch_at_slot(31)).to.equal(
                    Math.floor(31 / contracts.Utils.SLOTS_PER_EPOCH),
                );
                expect(contracts.Utils.compute_epoch_at_slot(32)).to.equal(
                    Math.floor(32 / contracts.Utils.SLOTS_PER_EPOCH),
                );
                expect(contracts.Utils.compute_epoch_at_slot(33)).to.equal(
                    Math.floor(33 / contracts.Utils.SLOTS_PER_EPOCH),
                );
            });
        });
        ...
        describe('merkle_root', function () {
            it('should return empty hash if array is empty', function () {
                expect(Uint8ArrayToHexString(contracts.Utils.merkle_root([]))).to.equal(
                    '0x'.concat(Array(32 * 2 + 1).join('0')),
                );
            });

            it('should return hash of single element if array.length equals 1', function () {
                expect(Uint8ArrayToHexString(contracts.Utils.merkle_root([EMPTY_BYTES32]))).to.equal(
                    '0x66687aadf862bd776c8fc18b8e9f8e20089714856ee233b3902a591d0d5f2925',
                );
            });

            it('should return hash of both elements concatenated if array.length equals 2', function () {
                expect(Uint8ArrayToHexString(contracts.Utils.merkle_root([EMPTY_BYTES32, EMPTY_BYTES32]))).to.equal(
            ...
            it('should return correct merkle hashes when more than 2 elements (4 * EMPTY_BYTES32)', function () {
                expect(
                    Uint8ArrayToHexString(
                        contracts.Utils.merkle_root([EMPTY_BYTES32, EMPTY_BYTES32, EMPTY_BYTES32, EMPTY_BYTES32]),
                    ),
            ...
            it('should return correct merkle hashes when more than 2 elements (random hashes - precalculated root)', function () {
                expect(Uint8ArrayToHexString(contracts.Utils.merkle_root(RANDOM_BYTES))).to.equal(
                    Uint8ArrayToHexString(RANDOM_BYTES_ROOT),
                );
            });
        });
        ...
        describe('is_valid_merkle_branch', function () {
            it('should verify a correct input (4 * EMPTY_BYTES32)', function () {
                ...
                const branch = [digest(EMPTY_BYTES32), digest([...digest(EMPTY_BYTES32), ...digest(EMPTY_BYTES32)])];
                const depth = 2;
                const index = 1;
                const root = contracts.Utils.merkle_root([
                ...
                expect(contracts.Utils.is_valid_merkle_branch(leaf, branch, depth, index, root)).to.be.true;
            });
        ...
        describe('hash_tree_root', function () {
            describe('fork_data', function () {
                it('should return correct fork data hash (empty fork data)', function () {
                    expect(
                        Uint8ArrayToHexString(
                            contracts.Utils.hash_tree_root__fork_data({
            ...
            describe('signing_data', function () {
                it('should return correct signing data hash (empty signing data)', function () {
                    expect(
                        Uint8ArrayToHexString(
                            contracts.Utils.hash_tree_root__signing_data({
            ...
            describe('block_header', function () {
                it('should return correct block header hash (EMPTY_BEACON_HEADER)', function () {
                    expect(
                        Uint8ArrayToHexString(contracts.Utils.hash_tree_root__block_header(EMPTY_BEACON_HEADER)),
                ...
                it('should return correct block header hash (normal header)', function () {
                    ...
                    expect(Uint8ArrayToHexString(contracts.Utils.hash_tree_root__block_header(header))).to.equal(
                        '0xe367fd4fce2eaa248d8d970ff2836a032b7c1dcea02b6f1bce831e13443e6f1e',
                    );
                });
            });
            ...
            describe('sync_committee', function () {
                it('should return correct sync committee hash', function () {
                    ...
                    expect(
                        Uint8ArrayToHexString(contracts.Utils.hash_tree_root__sync_committee(sync_committee)),
        ...
        describe('compute', function () {
            describe('fork_data_root', function () {
                it('should return correct fork data hash (empty fork data, same as hash_tree_root)', function () {
                    expect(
                        Uint8ArrayToHexString(contracts.Utils.compute_fork_data_root(EMPTY_BYTES4, EMPTY_BYTES32)),
                    ).to.equal('0xf5a5fd42d16a20302798ef6ed309979b43003d2320d9f0e8ea9831a92759fb4b');
            ...
            describe('domain', function () {
                it('should properly hash, calculate and return domain', function () {
                    ...
                    expect(
                        Uint8ArrayToHexString(
                            contracts.Utils.compute_domain(domain_type, fork_version, genesis_validators_root),
                        ),
            ...
            describe('signing_root', function () {
                it('should return correct signing data hash (empty fork data, same as hash_tree_root)', function () {
                    ...
                    expect(
                        Uint8ArrayToHexString(contracts.Utils.compute_signing_root(EMPTY_BEACON_HEADER, domain)),
