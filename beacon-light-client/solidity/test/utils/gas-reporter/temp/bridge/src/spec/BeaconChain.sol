...
contract BeaconChain is MerkleProof {
  ...
  function get_power_of_two_ceil(uint256 x) internal pure returns (uint256) {
    if (x <= 1) return 1; //gas-reporter #157
    else if (x == 2) return 2; //gas-reporter #158
    else return 2 * get_power_of_two_ceil((x + 1) >> 1); //gas-reporter #159
  }

  function to_little_endian_64(uint64 value) internal pure returns (bytes8 r) {
    return bytes8(reverse64(value)); //gas-reporter #163
  }

  function reverse64(uint64 input) internal pure returns (uint64 v) {
    v = input; //gas-reporter #167

    // swap bytes
    v = ((v & 0xFF00FF00FF00FF00) >> 8) | ((v & 0x00FF00FF00FF00FF) << 8); //gas-reporter #170

    // swap 2-byte long pairs
    v = ((v & 0xFFFF0000FFFF0000) >> 16) | ((v & 0x0000FFFF0000FFFF) << 16); //gas-reporter #173

    // swap 4-byte long pairs
    v = (v >> 32) | (v << 32); //gas-reporter #176
  }
}