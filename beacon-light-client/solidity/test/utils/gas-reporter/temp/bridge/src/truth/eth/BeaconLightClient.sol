...
contract BeaconLightClient is BeaconChain, BLSVerify {
  ...
  function state_root() public view returns (bytes32) {
    return finalized_header.state_root; //gas-reporter #124
  }
    ...
    payable
  {
    ...
    require(update.finalized_header.slot > finalized_header.slot, '!new'); //gas-reporter #163

    finalized_header = update.finalized_header; //gas-reporter #165
    prev_sync_committee_root = update.next_sync_committee_root; //gas-reporter #166

    emit FinalizedHeaderImported(update.finalized_header); //gas-reporter #168
  }
  ...
  function verify_signed_header(
    ...
    bytes32 signing_root = compute_signing_root(header, domain); //gas-reporter #186
