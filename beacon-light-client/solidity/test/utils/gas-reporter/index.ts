        ...
        for (let z = j + 1; z < contract.length; z++) {
          ...
          if (
            [
              ...
              'unchecked {',
              'unchecked{',
            ].some(v => line2.trim().startsWith(v))
          ) {
            ...
            start2 = [
              ...
              line2.indexOf('while ('),
              line2.indexOf('while('),
              line2.indexOf('assembly ('),
              line2.indexOf('assembly('),
              line2.indexOf('unchecked ('),
              line2.indexOf('unchecked('),
            ].filter(x => x !== -1)[0];
          ...
          if (
            contract[z - 1].trim().startsWith('return ') ||
            line2.trim().startsWith('return ')
          )
            continue;
          keyLines.add(z);
        }
...
tempContracts.map((contract, i) => {
  ...
  for (let keyLine of contractKeyLines) {
    contract[keyLine] = contract[keyLine].concat(
      ` //gas-reporter #${keyLine + 1}`,
    );
  }
});
