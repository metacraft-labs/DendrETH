...
template AggregateSubtractVerify(N) {
  var J = 2;
  var K = 7;
  ...
  // proof
  signal input negpa[N][2][6];
  signal input pb[N][2][2][6];
  signal input pc[N][2][6];
  ...
  for(var index = 0; index < N; index++) {
    groth16Verifier[index] = verifyProof(1);
    ...
    prevCommitments[index] = Pedersen(161);

    prevCommitments[index].in[0] <== maxActivationEpochs[index];
    prevCommitments[index].in[1] <== minExitEpochs[index];

    for(var j = 0; j < J; j++) {
      for(var k = 0; k < K; k++) {
        prevCommitments[index].in[2 + j * K + k]  <== points[index][j][k];
      }
    }

    prevCommitments[index].in[16] <== state_root;

    for(var i = 0; i < 6; i++) {
      for(var j = 0; j<2;j++) {
        for(var idx = 0; idx < 6; idx++) {
          prevCommitments[index].in[i * 12 + j * 6 +idx] <== zeroOnFirst * negalfa1xbeta2[i][j][idx];
        }
      }
    }
    ...
    for(var i = 0; i < 2; i++) {
      for(var j = 0; j < 2; j++) {
        for(var idx = 0; idx < 6; idx++) {
          prevCommitments[index].in[i * 12 + j * 6 + idx] <== zeroOnFirst * gamma2[i][j][idx];
          prevCommitments[index].in[i * 12 + j * 6 + idx] <== zeroOnFirst * delta2[i][j][idx];
          prevCommitments[index].in[i * 12 + j * 6 + idx] <== zeroOnFirst * IC[i][j][idx];
        }
      }
    }

    groth16Verifier[index].pubInput[0] <== prevCommitments[index].out[0];
