...
template LightClient(N) {
  ...
  component isValidMerkleBranch_finality = IsValidMerkleBranch(6);
  ...
  for(var i = 0; i < 256; i++) {
    isValidMerkleBranch_finality.leaf[i] <== finalized_header_root[i];
    isValidMerkleBranch_finality.root[i] <== state_root[i];
  }
