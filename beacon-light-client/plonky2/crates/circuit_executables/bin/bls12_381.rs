use anyhow::Result;
use circuits::{bls12_381_circuit::BlsCircuitTargets,serialization::targets_serialization::ReadTargets};
...
async fn async_main() -> Result<()> {
    ...
    let circuit_data = load_circuit_data_starky(&format!("{CIRCUIT_DIR}/{CIRCUIT_NAME}"));
    let target_bytes =
        read_from_file(&format!("{}/{}.plonky2_targets", CIRCUIT_DIR, CIRCUIT_NAME))?;
    let mut target_buffer = Buffer::new(&target_bytes);

    let targets = BlsCircuitTargets::read_targets(&mut target_buffer).unwrap();
...
async fn get_final_exp_proof(
    ...
    let final_exp_circuit_data =
        load_common_circuit_data_starky(&format!("{CIRCUIT_DIR}/final_exponentiate_circuit"));
    let final_exp_proof =
...
async fn get_fp12_mul_proof(
    ...
    let fp12_mul_circuit_data = load_common_circuit_data_starky(&format!("{CIRCUIT_DIR}/fp12_mul"));
    let fp12_mul_proof =
        ProofWithPublicInputs::<GoldilocksField, PoseidonGoldilocksConfig, 2>::from_bytes(
...
async fn get_miller_loop_proofs(
    ...
    let miller_loop_circuit_data =
        load_common_circuit_data_starky(&format!("{CIRCUIT_DIR}/miller_loop"));
...
async fn get_pairing_precomp_proofs(
    ...
    let pairing_precomp_circuit_data =
        load_common_circuit_data_starky(&format!("{CIRCUIT_DIR}/pairing_precomp"));
