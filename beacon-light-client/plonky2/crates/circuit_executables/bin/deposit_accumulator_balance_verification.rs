...
use circuits::{
    ...
    deposits_accumulator_balance_aggregator::{
        ...
        inner_level::DepositAccumulatorBalanceAggregatorInnerLevel,
    },
...
enum Targets<const VALIDATORS_COUNT: usize, const WITHDRAWAL_CREDENTIALS_COUNT: usize> {
    FirstLevel(Option<CircuitTargetType<DepositAccumulatorBalanceAggregatorFirstLevel>>),
...
async fn async_main() -> Result<()> {
    ...
    let matches = CommandLineOptionsBuilder::new("deposit_accumulator_balance_verification")
    ...
    let config = parse_balance_verification_command_line_options(&matches); //TODO: Reuse? for deposists?
...
async fn process_first_level_task(
    ...
    queue_item: Item,
    circuit_data: &CircuitData<GoldilocksField, PoseidonGoldilocksConfig, 2>,
    targets: &CircuitTargetType<DepositAccumulatorBalanceAggregatorFirstLevel>,
    protocol: &str,
) -> Result<()> {
...
>() -> Result<Targets<VALIDATORS_COUNT, WITHDRAWAL_CREDENTIALS_COUNT>, anyhow::Error> {
    ...
    Ok(Targets::FirstLevel(Some(
        deserialize_circuit_target::<DepositAccumulatorBalanceAggregatorFirstLevel>(
            &mut target_buffer,
        )
        .unwrap(),
    )))
...
>(
    ...
    Ok(
        Targets::<VALIDATORS_COUNT, WITHDRAWAL_CREDENTIALS_COUNT>::InnerLevel(Some(
            CircuitTargetType::<DepositAccumulatorBalanceAggregatorInnerLevel>::deserialize(
                &mut target_buffer,
            )
            .unwrap(),
        )),
