...
(async function () {
  ...
  const validatorsViewDU = ssz.deneb.BeaconState.fields.validators.toViewDU(
  ...
  console.log(
    'validators root',
    bytesToHex(newValidatorsTree.getNode(1n).root),
  );
  ...
  const hashTreeRoot2 = bytesToHex(
    ssz.phase0.Validator.hashTreeRoot(validator),
  );
