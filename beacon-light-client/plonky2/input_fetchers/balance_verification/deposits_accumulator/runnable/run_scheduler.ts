...
(async () => {
  ...
  for (
    let chunkIdx = 0;
    chunkIdx < Math.floor((deposits.length + CIRCUIT_SIZE - 1) / CIRCUIT_SIZE);
    chunkIdx++
  ) {
        ...
        balancesInput.balancesLeaves.push(
          bytesToHex(
            balancesTree.getNode(
              gindexFromIndex(BigInt(validatorIndices[idx]) / 4n, 39n),
            ).root,
          ),
        );
        balancesInput.balancesProofs.push(balancesProofs[idx]);
        balancesInput.validatorDepositIndexes.push(
          // TODO: delete this, we get this from the deposit
  ...
  for (let level = 1; level < 24; level++) {
    ...
    const range = [
      ...new Array(
        Math.ceil(deposits.length / CIRCUIT_SIZE / 2 ** level),
      ).keys(),
    ];
  ...
  async function scheduleFirstLevelTasks(
    ...
    for (let i = 0; i < balancesInputs.length; i++) {
      ...
      queue.addItem(db, new Item(Buffer.from(buffer)));
    }
  }
})();
