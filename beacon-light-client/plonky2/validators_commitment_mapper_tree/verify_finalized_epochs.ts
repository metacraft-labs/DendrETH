...
async function getFirstNonMissingSlotInEpoch(api: BeaconApi, epoch: number): Promise<number> {
  for (let relativeSlot = 0; relativeSlot < 31; ++relativeSlot) {
    ...
    try {
      const status = await api.pingEndpoint(`/eth/v1/beacon/states/${slot}/root`);
      if (status === 200) {
        return slot;
      }
  ...
  eventSource.on('finalized_checkpoint', async (event: any) => {
    ...
    try {
      ...
      if (validatorsRoot !== storedValidatorsRoot) {
        ...
        await scheduler.setValidatorsLength(BigInt(epoch), beaconState.validators.length);
      }
...
async function nodesAreSame(redis: Redis, newValidatorsTree: Tree, gindex: bigint, epoch: bigint): Promise<boolean> {
  ...
  const lastChangeEpoch = redis.getLatestEpoch(`${CONSTANTS.validatorProofKey}:${redisGindex}`, epoch);
  const node = await redis.get(`${CONSTANTS.validatorProofKey}:${lastChangeEpoch}`);
...
async function getValidatorsDiff(redis: Redis, newBeaconState: BeaconState, epoch: bigint): Promise<IndexedValidator[]> {
  ...
  for (let depth = 0; depth < 40; ++depth) {
    ...
    for (const changedNodeGindex of changedNodes) {
      ...
      for (const childGindex of [2n * changedNodeGindex, 2n * changedNodeGindex + 1n]) {
        if (!(await nodesAreSame(redis, newValidatorsTree, childGindex, epoch))) {
