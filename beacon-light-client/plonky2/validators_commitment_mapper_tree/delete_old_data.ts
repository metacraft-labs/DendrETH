...
(async () => {
  ...
  const validatorKeys = await redis.getAllKeys(
    `${validator_commitment_constants.validatorKey}:*:${validator_commitment_constants.epochLookupKey}`,
  );
  const validatorProofKeys = await redis.getAllKeys(
    `${validator_commitment_constants.validatorProofKey}:*:${validator_commitment_constants.epochLookupKey}`,
  );
  const keys = [...validatorKeys, ...validatorProofKeys].map(key =>
    key.substring(0, key.lastIndexOf(':')),
  );

  const deleted = await Promise.all(
    keys.map(async key => redis.pruneOldEpochs(key, options['oldest-epoch'])),
  );
