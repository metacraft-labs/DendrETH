...
pub fn main() {
    ...
    let file_path_attestations_out = 
        "/home/stefan/code/repos/metacraft-labs/DendrETH/casper-finality-proofs/data/full_validator_set_out.json";

    let  validators_raw: Vec<Validator> = read_validator_data(file_path_attestations);

    // let mut poseidon_validator_obj_vec = vec![];
    let mut validators_hashed: Vec<HashOut<GoldilocksField>> = vec![];
