...
export async function uploadVerifierConstantineContract(
  network: string,
  cosmos: CosmosClientWithWallet,
) {
  const { wasmContractPath } = await getCosmosContractArtifacts(
...
export async function instantiateVerifierConstantineContract(
  uploadReceipt: UploadResult,
  initHeaderRoot: string,
  domain: string,
  cosmos: CosmosClientWithWallet,
